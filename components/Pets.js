...
const Pets = () => {
  ...
  <section className='bg-pets bg-center py-6 overflow-hidden'>
    <div className='flex flex-col lg:flex-row'>
      ...
      {/* pet details */}
      <div className='lg:w-[30%] flex flex-col justify-items-center items-end pb-6 lg:py-2 mx-auto lg:mx-0'>
        ...
        <div className='flex flex-wrap gap-4 justify-center lg:justify-end lg:-mr-6'>
          {pets.map((pet, index) => {
            ...
            className='cursor-pointer relative' key={index}>
              {/* overlay */}
              <div 
              className={`w-full h-full absolute rounded-full ${petIndex === index ? 'border-2 border-white' : 'bg-black/40' }`}></div>
              <Image 
              src={pet.image} 
              width={95} 
              height={95} 
